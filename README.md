...
The pipeline needs [uv](https://docs.astral.sh/uv/). Downloads are cached in `pipeline/cache/`, so reruns are fast. The newer map areas read USGS's bulk NHDPlus HR files, about 125–150 MB zipped per 4-digit HUC, downloaded there on first use. Pass `--refresh` to refetch everything.
